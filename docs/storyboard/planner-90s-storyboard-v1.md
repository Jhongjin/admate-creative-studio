...
AdMate는 미디어플래너를 대체하지 않는다. Compass가 정책/가이드 근거 확인의 출발점을 잡고, Sentinel이 세팅 검수와 운영 이상 감지 신호를 정리한다. Lens는 캡처와 증빙을 보고 가능한 흐름으로 묶고, Foresight는 다음 판단을 위한 예측 가설과 분석 관점을 제안한다. Agent Core는 운영자의 판단이 다음 기준으로 돌아오도록 연결한다.
...
| P5 | 0:31-0:46 | Sentinel card + setup validation/anomaly signal | Sentinel은 세팅 검수와 운영 이상 감지를 돕는다 | voice-over only | 승인 없는 차단/집행 표현 금지 |
| P6 | 0:46-0:58 | Sanitized capture proof tiles | Lens는 캡처와 증빙을 맥락과 함께 정리한다 | voice-over only | 실제 내부 화면/계정 정보 금지 |
| P7 | 0:58-1:12 | Foresight hypothesis card | Foresight는 분석 관점과 다음 액션 가설을 제안한다 | voice-over only | 성과 보장/실제 수치 금지 |
...
Compass는 브리프와 운영 조건을 읽고 정책/가이드 근거 확인의 출발점을 잡는다. Sentinel은 세팅 누락, 승인 필요 항목, 운영 중 다시 봐야 할 이상 신호를 정리한다. Lens는 캡처와 증빙을 보고 가능한 흐름으로 남긴다. Foresight는 성과를 약속하지 않고 다음 판단을 위한 분석 관점과 가설을 제안한다.
